...
namespace Hospitality
{
    public static class BedStatsDrawer
    {
        ...
        public static void DoBedInfos(Rect windowRect, Building_GuestBed bed)
        {
            ...
            if (!string.IsNullOrEmpty(bed.Stats.textNextTitleReq))
            {
        ...
        {
            var stackElements =
                from titleDef in DefDatabase<RoyalTitleDef>.AllDefsListForReading
                orderby titleDef.seniority
                let titleLabel = titleDef.GetLabelCapForBothGenders()
                let possibleFactions = GuestUtility.DistinctFactions
                    .Where(f =>
                        f?.def?.RoyalTitlesAllInSeniorityOrderForReading != null &&
                        f.def.RoyalTitlesAllInSeniorityOrderForReading.Contains(titleDef))
                    .ToArray()
                let accepted = bed.Stats?.metRoyalTitles != null && bed.Stats.metRoyalTitles.Contains(titleDef)
                ...
                select new GenUI.AnonymousStackElement
                {
                    drawer = delegate (Rect r)
                    {
                        ...
                        Widgets.Label(
                            new Rect(labelRect.x + labelRect.height + 5f, labelRect.y, labelRect.width - 10f, labelRect.height),
                            titleLabel);
                        GUI.color = guiColor;
                    },
